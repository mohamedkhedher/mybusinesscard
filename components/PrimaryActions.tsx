...
export default function PrimaryActions({ compact = false }: PrimaryActionsProps) {
    return (
        <div className={`relative z-10 w-full max-w-md mx-auto px-4 ${compact ? "" : "mb-4"}`}>
            {/* Primary CTA */}
            {!compact && (
                <a
                    href="/api/vcf"
                    ...
                    className="btn-primary flex items-center justify-center gap-2.5 w-full py-4 text-base mb-3"
            ...
            {/* Secondary row */}
            <div className={`grid ${compact ? "grid-cols-2 gap-2.5" : "grid-cols-4 gap-2.5"}`}>
                {/* WhatsApp */}
                <a
                    href={`https://wa.me/${PROFILE.phoneDigitsIntl}?text=${waMessage}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="btn-icon flex-col gap-1.5 py-3.5 px-2"
